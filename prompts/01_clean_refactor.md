# Task
Refactor and rewrite the provided `app.R` into a clean, readable, well-commented single-file Shiny app.

## Requirements
- Keep EXACT same functionality as current app:
  - TOC search with progress bar
  - dataset selection (DT row selection)
  - DSD loading (restatapi)
  - dimension browser (select dimension -> show codes/labels)
  - SDMX 2.1 query builder (manual URL; default SDMX-CSV)
  - clipboard copy (fallback using execCommand)
  - data preview (download SDMX-CSV and show first rows)
  - download button (SDMX-CSV)
  - reset button clears EVERYTHING:
    - search field + search results table
    - selected dataset
    - DSD + dim selector + query builder inputs
    - final URL
    - preview table and status text

## Code constraints
- Use only these packages: shiny, DT, restatapi
- Do NOT add new dependencies
- Keep it as a single file `app.R`
- Remove duplicated code (e.g. only one downloadHandler)
- Remove debug `cat()` observers
- Keep helper functions at top (e.g. match_query_groups, fetch_sdmx_csv, parse_csv_lines)
- Organize server into clearly separated sections using large comment headers

## Output
Return ONLY the full content of the rewritten `app.R` (no explanation).
